...
async function loadExpenses(page) {
  try {
    ...
    data.expenses.forEach((expense) => {
      ...
      const user_name_surname = expense.name + " " + expense.surname;
      row.innerHTML = `
                <td>${expense.date}</td>
                <td><p>${user_name_surname}</p></td>
                ...
                <td>
                    <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" onclick="openEditPopup(${
                      expense.id
                    }, '${expense.date}', '${expense.user_id}', '${
        expense.category
      }', '${expense.descr}', ${expense.amount})">
                ...
                <td>
                    <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" onclick="deleteExpense(${
                      expense.id
                    }, '${expense.date}', '${user_name_surname}', '${
        expense.category
      }', '${expense.descr}', ${expense.amount})">
  ...
  try {
    ...
    data.forEach((expense) => {
      ...
      userItem.innerHTML = `
                ...
                <span class="name">${expense.name} <br> ${
        expense.surname
      }</span>
                <div class="amounts">
                    <div class="amount-row">
                        ...
                        <span class="week-value">${
                          expense.weeklyTotal !== null
                            ? expense.weeklyTotal.toFixed(2)
                            : "0.00"
                        }€</span>
                    </div>
                    <div class="amount-row">
...
async function openCreatePopup() {
  ...
  formData.onsubmit = async (e) => {
    ...
    const selectedUsers = Array.from(
      document.querySelectorAll('input[name="foruser"]:checked')
...
async function openEditPopup(id, date, userId, category, descr, amount) {
  await loadUsers(); // Loads users when the popup is opened
  await loadCategories(); // Loads catgories when the popup is opened
  ...
  document.getElementById("user").value = userId; // Pre-fills the userr
